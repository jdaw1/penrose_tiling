			...
			if( NumOutputablePaths > 0 )
			{
				...
				if( pathStatP -> pathClosed )
				{
					...
					sprintf(scratchString,
						"  /WidthMax %.12lf  /HeightMax %.12lf  /RadiusMinMin %.12lf  /RadiusMaxMax %.12lf",
						pathStatP->widthMax  / tlngP->edgeLength,
						pathStatP->heightMax / tlngP->edgeLength,
						pathStatP->radiusMin / tlngP->edgeLength,
			...
			else
			{
				...
				if( pathStatP->pathClosed)
				{
					...
					(*numCharsThisFileP) += fprintf(fp,
						...
						"\t#N/A"  "\t#N/A"  "\t#N/A"  "\t#N/A"  // radiusMin, radiusMax, widthMax, heightMax
						"\tOpen"  // InteriorsConsistent. String rather than Boolean faciltates Excel AND() function.
					);  // fprintf()
